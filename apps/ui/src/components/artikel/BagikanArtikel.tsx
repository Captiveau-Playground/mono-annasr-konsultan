"use client"

import { LinkIcon } from "lucide-react"
import { toast } from "sonner"

type IkonProps = { className?: string }

function IkonWhatsApp({ className = "size-4" }: IkonProps) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden="true"
      className={className}
    >
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413Z" />
    </svg>
  )
}

function IkonFacebook({ className = "size-4" }: IkonProps) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden="true"
      className={className}
    >
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  )
}

function IkonLinkedin({ className = "size-4" }: IkonProps) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden="true"
      className={className}
    >
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 1 1 0-4.124 2.062 2.062 0 0 1 0 4.124zM7.119 20.452H3.555V9h3.564v11.452z" />
    </svg>
  )
}

const OPSI_SHARE = [
  {
    nama: "WhatsApp",
    Ikon: IkonWhatsApp,
    warna: "bg-[#25d366] hover:brightness-105",
  },
  {
    nama: "Facebook",
    Ikon: IkonFacebook,
    warna: "bg-[#1877f2] hover:brightness-105",
  },
  {
    nama: "LinkedIn",
    Ikon: IkonLinkedin,
    warna: "bg-[#0a66c2] hover:brightness-105",
  },
]

/** Baris share di bawah isi artikel — rata kiri, tombol brand + salin link. */
export function BagikanArtikel() {
  const url = () => window.location.href

  const bukaShare = (link: string) => {
    window.open(link, "_blank")
  }

  const salinLink = async () => {
    try {
      await navigator.clipboard.writeText(url())
      toast.success("Link artikel berhasil disalin.")
    } catch {
      toast.error("Gagal menyalin link.")
    }
  }

  return (
    <div className="border-border mx-auto mt-12 max-w-xl border-t pt-8">
      <p className="text-foreground font-[family-name:var(--font-heading)] text-sm font-semibold">
        Bagikan artikel ini
      </p>
      <div className="mt-4 flex flex-wrap items-center gap-3">
        {OPSI_SHARE.map((opsi) => {
          const Ikon = opsi.Ikon

          const buatLink = () => {
            if (opsi.nama === "WhatsApp")
              return `https://api.whatsapp.com/send?text=${encodeURIComponent(
                `Baca artikel CV. AN NASR KONSULTAN\n${url()}`
              )}`
            if (opsi.nama === "Facebook")
              return `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(
                url()
              )}`

            return `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(
              url()
            )}`
          }

          return (
            <button
              key={opsi.nama}
              type="button"
              onClick={() => bukaShare(buatLink())}
              className={`inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all ${opsi.warna}`}
            >
              <Ikon />
              {opsi.nama}
            </button>
          )
        })}
        <button
          type="button"
          onClick={salinLink}
          className="border-border bg-card text-foreground hover:border-primary/40 hover:text-primary inline-flex items-center gap-2 rounded-full border px-5 py-2.5 text-sm font-medium shadow-sm transition-colors"
        >
          <LinkIcon className="size-4" />
          Salin Link
        </button>
      </div>
    </div>
  )
}
